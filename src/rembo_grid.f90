module rembo_grid
    ! This module holds routines to manage the gridded coordinates of a given rembo domain 

    use rembo_defs, only : sp, dp, wp, pi, rgrid_class
    use nml 
    use ncio 

    implicit none 

    interface rembo_init_grid
        module procedure rembo_init_grid_fromfile
!         module procedure rembo_init_grid_fromnml    ! Ambiguous interface with rembo_init_grid_fromfile
        module procedure rembo_init_grid_fromname
        module procedure rembo_init_grid_fromgrd
        module procedure rembo_init_grid_fromaxes
        module procedure rembo_init_grid_fromopt
    end interface 
    
    private 

    public :: rembo_init_grid 
    public :: rembo_init_grid_fromfile
    public :: rembo_init_grid_fromnml
    public :: rembo_init_grid_fromname
    public :: rembo_init_grid_fromaxes
    public :: rembo_init_grid_fromopt 
    public :: rembo_init_grid_fromgrd
    public :: rembo_grid_write

contains 
    
    subroutine rembo_init_grid_fromfile(grd,filename,grid_name,xnm,ynm,lonnm,latnm)

        implicit none 

        type(rgrid_class), intent(INOUT) :: grd 
        ...
        select case(trim(units))
            ...
            case DEFAULT 
                write(*,*) "rembo_init_grid_fromfile:: &
                ...
                if ( nc_exists_var(filename,crs_name) .and. trim(crs_name) .eq. "crs" ) then
                    ...
                    select case(trim(grd%mtype))
                        ...
                        case DEFAULT 
                            ...
                            write(*,*) "rembo_init_grid_fromfile:: Warning: unsupported grid projection. &
    ...
    end subroutine rembo_init_grid_fromfile

    subroutine rembo_init_grid_fromnml(grd,filename,grid_name)
        ...
        if (x0 .ne. -9999.0) then 
            ...
            call rembo_init_grid_fromopt(grd,grid_name,units,x0=x0,dx=dx,nx=nx,y0=y0,dy=dy,ny=ny)
        else
            ! Call to generate grid centered on 0/0
            call rembo_init_grid_fromopt(grd,grid_name,units,dx=dx,nx=nx,dy=dy,ny=ny)
        end if 
    ...
    end subroutine rembo_init_grid_fromnml

    subroutine rembo_init_grid_fromname(grd,grid_name)
        ...
        select case(trim(grid_name))
            ...
            case DEFAULT
                write(*,*) "rembo_init_grid_fromname:: error: grid name not recognized: "//trim(grid_name)
                stop 
    ...
    end subroutine rembo_init_grid_fromname

    subroutine rembo_init_grid_fromgrd(grd,grid_name,grd_src,dx,dy)
        ...
        type(rgrid_class), intent(INOUT) :: grd 
        character(len=*),  intent(IN)    :: grid_name 
        type(rgrid_class), intent(IN)    :: grd_src
        ...
        write(*,*) "rembo_init_grid_fromgrd:: Error: not yet implemented - see rembo_grid.f90."
        stop 
...
!         if (present(dx)) 
    ...
    end subroutine rembo_init_grid_fromgrd

    subroutine rembo_init_grid_fromaxes(grd,grid_name,xc,yc,lon,lat,area)
    ...
    end subroutine rembo_init_grid_fromaxes
    
    subroutine rembo_init_grid_fromopt(grd,grid_name,units,x0,dx,nx,y0,dy,ny,lon,lat,area)
        ...
        select case(trim(units))
            ...
            case DEFAULT 
                ...
                write(*,*) "rembo_init_grid:: Error: units of input grid parameters &
    ...
    end subroutine rembo_init_grid_fromopt
    ...
    end subroutine rembo_grid_write

end module rembo_grid