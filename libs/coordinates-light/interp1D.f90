...
module interp1D
    ...
    interface interp_spline
        module procedure interp_spline_float, interp_spline_dble
    end interface
...
!         end do
    ...
    function interp_spline_float(x,y,xout) result(yout)
        ...
        real(dp), allocatable :: y_dp(:)
        real(dp), allocatable :: xout_dp(:) 
        real(dp), allocatable :: yout_dp(:) 
        ...
        allocate(x_dp(n))
        allocate(y_dp(n))
        allocate(xout_dp(nout))
        allocate(yout_dp(nout))

        x_dp = real(x,sp)
        y_dp = real(y,sp)
        xout_dp = real(xout,dp)
        
        yout_dp = interp_spline_dble(x_dp,y_dp,xout_dp)

        yout = real(yout_dp,sp)

        return
        
    end function interp_spline_float

    function interp_spline_dble(x,y,xout) result(yout)
        ...
        do i = 1, nout 
            if (xout(i) .lt. x(1)) then
                dx = x(1)-xout(i)
                uh = x(1)+dx
                yh = ispline_dble(uh,x,y,b,c,d,n)
                yout(i) = y(1) + (y(1)-yh)
                ...
                yout(i) = ispline_dble(xout(i), x, y, b, c, d, n)
            end if 
        end do 

        return

    end function interp_spline_dble

    subroutine spline_dble (x, y, b, c, d, n)
        !======================================================================
    ...
    end subroutine spline_dble

    function ispline_dble(u, x, y, b, c, d, n)
        !======================================================================
        ! SOURCE: http://ww2.odu.edu/~agodunov/computing/programs/book2/Ch01/spline.f90
        ...
        !*
        dx = u - x(i)
        ispline_dble = y(i) + dx*(b(i) + dx*(c(i) + dx*d(i)))

        return
        
    end function ispline_dble
